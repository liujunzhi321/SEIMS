...
MONGO_EXPORT int gridfs_init( mongo *client, const char *dbname, const char *prefix,
    gridfs *gfs ) {
...
MONGO_EXPORT int gridfile_writer_done( gridfile *gfile ) {
    ...
    response = gridfs_insert_file2( gfile->gfs, gfile->remote_name, gfile->id,
                                   gfile->length, gfile->content_type, gfile );
...
MONGO_EXPORT void gridfile_set_metadata( gridfile *gfile, bson* meta ) {
    gfile->meta = ( bson * )bson_malloc( sizeof( bson ) );
    bson_copy(gfile->meta, meta);
}

/*
MONGO_EXPORT void gridfile_set_metadata2( gridfile *gfile, bson* meta ) {
    gfile->meta = ( bson * )bson_malloc( sizeof( bson ) );
    bson_copy(gfile->meta, meta);
    ...
    mongo_cursor cur;
    mongo_cursor_set_query(&cur, b);
    mongo_cursor_next(&cur);

    bson_append_bson(cur.fields, "metadata", gfile->meta);

    mongo_update(gfile->gfs->client, gfile->gfs->files_ns, &b, cur.fields, 0);
