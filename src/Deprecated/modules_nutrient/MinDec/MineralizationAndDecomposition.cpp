...
MineralizationAndDecomposition::MineralizationAndDecomposition(void) : m_nLayers(3), m_size(-1), m_RateConst(0.00001f),
                                                                       m_HumNF(0.02f), m_RateCoeH(0.002f),    //input
                                                                       m_RateCoeR(0.05f), m_conSoluP(5.0f),
                                                                       m_Density(NULL), m_SoilT(NULL), m_SOMO(NULL),
                                                                       m_FieldCap(NULL), m_Depth(NULL),
                                                                       m_TF(NULL), m_WF(NULL), m_ActOrgN(NULL),
                                                                       m_StaOrgN(NULL), m_TraOrgN(NULL), m_orgCar(
                NULL),                                                              //output
                                                                       m_MinHumN(NULL), m_Res(NULL), m_Nitrate(NULL),
                                                                       m_SoluP(NULL), m_CNRat(NULL), m_CPRat(NULL),
                                                                       m_RateConRD(NULL), m_ResComF(NULL),
                                                                       m_MinFreOrgN(NULL), m_DecFreOrgN(NULL),
                                                                       m_actOrgP(NULL), m_conHumOrgN(NULL),
                                                                       m_conActOrgN(NULL), m_conStaOrgN(NULL),
                                                                       m_HumOrgN(NULL), m_FreOrgN(NULL),
                                                                       m_humOrgP(NULL), m_FreOrgP(NULL),
                                                                       m_staOrgP(NULL), m_minHumP(NULL),
                                                                       m_minFreOrgP(NULL), m_decFreOrgP(NULL)
{
}
...
bool MineralizationAndDecomposition::CheckInputData(void)
{
    if (this->m_size <= 0)
    {
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
    ...
    {
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the fraction of humic nitrogen in the active pool.");
        return false;
    }
    if (this->m_RateCoeH <= 0)
    {
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the rate coefficient for mineralization of the humus active organic nutrients.");
    }
    if (this->m_RateCoeR <= 0)
    {
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the rate coefficient for mineralization of the residue fresh organic nutrients.");
    }
    if (this->m_conSoluP <= 0)
    {
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the concentration of solution phosphorus all layers.");
        return false;
    }
    ...
    if (m_Density == NULL)
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the bulk density of the layer.");
    //if(m_orgCar == NULL)
    //{
    ...
    if (m_SoilT == NULL)
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the soil temperature.");
    if (m_SOMO == NULL)
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the water content on a given day.");
    if (m_FieldCap == NULL)
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the water content at field capacity.");
    if (m_Depth == NULL)
        throw ModelException("MineralizationAndDecomposition", "CheckInputData",
                             "You have not set the depth of the layer.");
    //if(m_som == NULL)
...
{
    if (this->m_size <= 0)
        throw ModelException("MineralizationAndDecomposition", "initialOutputs",
                             "The cell number of the input can not be less than zero.");
