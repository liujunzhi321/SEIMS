...
InorganicPSorption::InorganicPSorption(void) : m_nLayers(3), m_size(-1), m_sloER(0.0006f), m_SoluP(NULL),  // input
                                               m_pai(NULL), m_actMinP(NULL), m_staMinP(NULL), m_tSolActP(NULL),
                                               m_tStaActP(NULL)  // output
{
...
bool InorganicPSorption::CheckInputData(void)
{
    if (m_size <= 0)
    {
        throw ModelException("InorganicPSorption", "CheckInputData",
    ...
    {
        throw ModelException("InorganicPSorption", "CheckInputData",
                             "The rslow equilibration rate constant of the input can not be less than zero.");
        return false;
    }
    if (m_SoluP == NULL)
        throw ModelException("InorganicPSorption", "CheckInputData",
                             "You have not set the amount of phosphorus in solution.");
    /*if(m_aftSolP == NULL)
...
{
    if (m_size <= 0)
        throw ModelException("InorganicPSorption", "initialOutputs",
                             "The cell number of the input can not be less than zero.");
