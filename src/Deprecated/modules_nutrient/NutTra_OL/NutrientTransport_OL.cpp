...
NutrientTransport_OL::NutrientTransport_OL(void) : m_nLayers(3), m_nCells(-1), m_aniExcF(0.2f), m_nitPC(0.26f),
                                                   m_Nitrate(NULL), m_porosity(NULL),
                                                   m_sr(NULL), m_ssr(NULL),
                                                   m_perc(NULL),                                             // nitrate input
                                                   m_clayFrac(NULL), m_Ammon(NULL),    // Ammonium input
                                                   m_parCoeP(165.f), m_phoPC(15.f),
                                                   m_SoluP(NULL),                  // soluble P input
                                                   m_CellWidth(-1), m_Depth(NULL), m_sedimentYield(NULL),
                                                   m_Density(NULL), m_ActOrgN(NULL), m_StaOrgN(NULL),
                                                   m_FreOrgN(NULL),   // organic N input
                                                   m_actMinP(NULL), m_staMinP(NULL), m_humOrgP(NULL),
                                                   m_FreOrgP(NULL),    // Attached P input
                                                   m_chWidth(NULL), m_streamLink(NULL), m_flowInIndex(NULL),
                                                   m_routingLayers(NULL), m_nRoutingLayers(-1),   // cell to cell
                                                   m_nsub(-1), m_subbasin(NULL),  // to channel input
                                                   m_satW(NULL), m_mobQ(NULL), m_conNit(NULL), m_surNit(NULL),
                                                   m_latNit(NULL), m_perNit(NULL),  // nitrate output
                                                   m_surAmm(NULL), m_conAmm(NULL),      // Ammonium output
                                                   m_conSed(NULL), m_enrN(NULL), m_conOrgN(NULL),
                                                   m_traOrgN(NULL),   // organic N output
                                                   m_surSolP(NULL), m_perPho(NULL),   // soluble P output
                                                   m_enrP(NULL), m_conAttP(NULL),
                                                   m_traAttP(NULL),    // Attached P output
                                                   m_sNittoCh(NULL), m_sNittoCh_T(0.f), m_sAmmtoCh(NULL),
                                                   m_sAmmtoCh_T(0.f), m_sSolPtoCh(NULL), m_sSolPtoCh_T(0.f),
                                                   m_orgNtoCh(NULL), m_orgNtoCh_T(0.f),
                                                   m_attPtoCh(NULL), m_attPtoCh_T(0.f), m_laNittoCh(NULL),
                                                   m_laNittoCh_T(0.f)   // to channel output
{
...
bool NutrientTransport_OL::CheckInputData(void)
{
    if (m_nCells <= 0)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
    ...
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "The phosphorus percolation coefficient can not be less than zero.");
        return false;
    }
    if (m_phoPC <= 0)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "The phosphorus soil partitioning coefficient can not be less than zero.");
        return false;
    }
    if (m_Nitrate == NULL)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of nitrate in layer ly.");
        return false;
    }
    if (m_Ammon == NULL)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of ammonium in layer ly.");
        return false;
    }
    if (m_porosity == NULL)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the soil porosity of the soil layer.");
        return false;
    }
    if (m_clayFrac == NULL)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the clay fraction of the soil layer.");
        return false;
    }
    if (m_sr == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the surface runoff generated on a given day.");
    if (m_ssr == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the water discharged from the layer by lateral flow.");
    if (m_perc == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of water percolating to the underlying soil layer on a given day.");

    if (m_CellWidth <= 0)
    {
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "The width (length) of cell can not be less than zero.");
        return false;
    }
    if (m_Depth == NULL)
    ...
    if (m_sedimentYield == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the sediment yield on a given day.");
    if (m_Density == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData", "You have not set the soil density.");
    if (m_ActOrgN == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of nitrogen in the active organic pool in layer ly.");
    if (m_StaOrgN == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of nitrogen in the stable organic pool.");
    if (m_FreOrgN == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of nitrogen in the fresh organic pool in layer ly.");
    if (m_SoluP == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of soluble phosphorus lost in surface runoff.");
    if (m_actMinP == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of phosphorus in the active mineral pool.");
    if (m_staMinP == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the amount of phosphorus in the stable mineral pool.");
    if (m_humOrgP == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the concentration of humic organic phosphorus in the layer.");
    if (m_FreOrgP == NULL)
        throw ModelException("NutrientTransport_OL", "CheckInputData",
                             "You have not set the phosphorus in the fresh organic pool in layer ly.");
    if (m_subbasin == NULL)
    {
...
{
    if (m_nCells <= 0)
        throw ModelException("NutrientTransport_OL", "initialOutputs",
                             "The cell number of the input can not be less than zero.");
    if (m_nsub <= 0)
    {
        map<int, int> subs;
...
void NutrientTransport_OL::GetValue(const char *key, float *value)
{
    ...
    else if (StringMatch(s, "LANITTOCH_T"))
    {
        *value = m_laNittoCh_T;
        ...
        throw ModelException("NutrientTransport_OL", "getResult", "Result " + s +
                                                                  " does not exist in NutrientTransport_OL method. Please contact the module developer.");
}
