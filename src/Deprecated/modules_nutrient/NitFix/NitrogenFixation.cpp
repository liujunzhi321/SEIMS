...
NitrogenFixation::NitrogenFixation(void) : m_nLayers(3), m_size(-1), m_frPHU(NULL), m_Nitrate(NULL), m_SOMO(NULL),
                                           m_FieldCap(NULL), m_demNit(NULL),  //input
...
bool NitrogenFixation::CheckInputData(void)
{
    if (m_size <= 0)
    {
        throw ModelException("NitrogenFixation", "CheckInputData",
    ...
    {
        throw ModelException("NitrogenFixation", "CheckInputData",
                             "You have not set the fraction of phu accumulated on a given day.");
        return false;
    }
    if (m_Nitrate == NULL)
    {
...
{
    if (m_size <= 0)
        throw ModelException("NitrogenFixation", "initialOutputs",
                             "The cell number of the input can not be less than zero.");
