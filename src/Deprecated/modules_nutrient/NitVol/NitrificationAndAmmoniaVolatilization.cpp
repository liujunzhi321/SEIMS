...
NitrificationAndAmmoniaVolatilization::NitrificationAndAmmoniaVolatilization(void) : m_size(-1), m_nLayers(3),
                                                                                     m_catEF(0.15f), m_SoilT(NULL),
                                                                                     m_SOMO(NULL),
                                                                                     m_Depth(NULL), //input
                                                                                     m_Wiltingpoint(NULL),
                                                                                     m_FieldCap(NULL), m_Ammon(NULL),
                                                                                     m_Nitrate(NULL),
                                                                                     m_midZ(NULL), m_nitvolTF(NULL),
                                                                                     m_nitWF(NULL), m_volDF(NULL),
                                                                                     m_nitR(NULL), m_volR(NULL),
                                                                                     m_nitvolAmmon(NULL),
                                                                                     m_nitLostF(NULL), m_volLostF(NULL),
                                                                                     m_conNitra(NULL),
                                                                                     m_conAmm(NULL) //output
{
...
bool NitrificationAndAmmoniaVolatilization::CheckInputData(void)
{
    ...
    if (m_Wiltingpoint == NULL)
        throw ModelException("NitVol", "CheckInputData",
                             "You have not set the amount of water held in the soil layer at wilting point water content.");
    if (m_FieldCap == NULL)
...
{
    if (this->m_size <= 0)
        throw ModelException("NitrificationAndAmmoniaVolatilization", "initialOutputs",
    ...
    for (int i = 0; i < m_nLayers; i++)
    {
        for (int j = 0; j < m_size; j++)
        {
            ...
            if (m_SOMO[hydroIndex][j] < r)
                m_nitWF[i][j] = (m_SOMO[hydroIndex][j] - m_Wiltingpoint[hydroIndex][j]) /
                                (0.25f * (m_FieldCap[hydroIndex][j] - m_Wiltingpoint[hydroIndex][j]));
            else
                m_nitWF[i][j] = 1.0f;
