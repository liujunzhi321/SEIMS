...
BMPReachFactory::BMPReachFactory(int scenarioId,int bmpId,int bmpType,
	string distribution,string parameter):BMPFactory(scenarioId,bmpId,bmpType,distribution,parameter)
{
...
void BMPReachFactory::loadBMP(string bmpDatabasePath)
{
	...
	if(!DBManager::IsTableExist(bmpDatabase,m_distribution))
		...
		"' does not exist or the there is not a table named '" + m_distribution + "' in BMP database.");
						...
						if(m_bmpType == BMP_TYPE_REACH)
						{
							...
							switch(m_bmpId)
							{
								...
								case BMP_TYPE_FLOWDIVERSION_STREAM:
									reach = new BMPReachFlowDiversion(bmpDatabasePath,m_parameter,reachId,reachConstructureId);
									break;
								case BMP_TYPE_RESERVOIR:
									reach = new BMPReachReservoir(bmpDatabasePath,m_parameter,reachId,reachConstructureId);
									break;
