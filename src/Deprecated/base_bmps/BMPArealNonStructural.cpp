...
BMPArealNonStructural::BMPArealNonStructural(string bmpDatabasePath,
					...
					string bmpName,
					string distribution,
					int scenario):BMPBase(bmpDatabasePath,bmpId,bmpName,BMP_TYPE_AREAL_NON_STRUCTURAL,parameterTableName)
{
	m_maxYear = 0;
...
void BMPArealNonStructural::loadParameters()
{
	...
	if(!DBManager::IsTableExist(bmpDatabase,m_parameterTableName))
		...
		"' does not exist or the there is not a table named '" + m_parameterTableName + "' in BMP database.");
...
void BMPArealNonStructural::loadOperations()
{
	...
	if(!DBManager::IsTableExist(bmpDatabase,m_distributionTableName))
		throw ModelException("BMPArealNonStructural","loadOperations","The BMP database '" + bmpDatabase + 
		"' does not exist or the there is not a table named '" + m_distributionTableName + "' in BMP database.");
					...
					it->second == scenario)				//exists and has same scenario
				{
					...
					if(m_bmpId == BMP_TYPE_CROP) //crop management, split to plant and harvest operation
					{
						...
						NonStructural::ManagementOperationPlant* plantOp = 
							new NonStructural::ManagementOperationPlant(location,year,month,day,getParameter(cropType),cropType);
					...
					{			
						...
						NonStructural::ManagementOperationFertilizer* ferOp = 
							new NonStructural::ManagementOperationFertilizer(location,year,month,day,getParameter(ferType),ferType,ferRate);
					...
					{			
						...
						NonStructural::ManagementOperationTillage* tillOp = 
							new NonStructural::ManagementOperationTillage(location,year,month,day,getParameter(tillCode),tillCode);
...
NonStructural::ManagementOperation* BMPArealNonStructural::getSecondOperation(int validCellIndex,int startYear,time_t currentTime)
{
...
NonStructural::ManagementOperation* BMPArealNonStructural::getOperation(int validCellIndex,int startYear,time_t currentTime)
{
	//get location id from raster file
	int location = int(m_currentScearioDistribution->getValue(validCellIndex));
