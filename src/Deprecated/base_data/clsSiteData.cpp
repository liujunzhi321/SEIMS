...
void clsSiteData::checkData(string typeName, time_t time, float value)
{
    if (!StringMatch(typeName, DataType_MinimumTemperature) && !StringMatch(typeName, DataType_MaximumTemperature) &&
        !StringMatch(typeName, DataType_MeanTemperature))
    {
        if (value < 0)
            throw ModelException("clsSiteData", "checkData",
                                 typeName + " data could not be less than 0." + toString(time, value));

        if (StringMatch(typeName, DataType_WindSpeed) && value > 50)
            throw ModelException("clsSiteData", "checkData", "Wind speed could not be larger than 50." +
                                                             toString(time, value));
        if (StringMatch(typeName, DataType_Precipitation) && value > 500)
            throw ModelException("clsSiteData", "checkData", "Wind speed could not be larger than 500." +
                                                             toString(time, value));
    }
    else
    {
        if (value < -90)
            throw ModelException("clsSiteData", "checkData",
                                 "Temperature could not be less than -90." + toString(time, value));
        if (value > 60)
            throw ModelException("clsSiteData", "checkData",
                                 "Temperature could not be larger than 60." + toString(time, value));
    }
}
