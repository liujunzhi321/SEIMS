...
bool MUSLE_I30::CheckInputData(void)
{
    ...
    if (m_cellSize <= 0)
        throw SEIMSException("MUSLE_I30", "CheckInputData",
                             "The dimension of the input data can not be less than zero.");
    if (m_cellWidth <= 0)
        throw SEIMSException("MUSLE_I30", "CheckInputData", "The cell width can not be less than zero.");
    ...
    if (this->m_flowacc == NULL)
        throw SEIMSException("MUSLE_I30", "CheckInputData", "The flow accumulation can not be NULL.");
    ...
    if (this->m_snowAccumulation == NULL)
        throw SEIMSException("MUSLE_I30", "CheckInputData", "The snow accumulation can not be NULL.");
    if (this->m_surfaceRunoff == NULL)
        throw SEIMSException("MUSLE_I30", "CheckInputData", "The surface runoff can not be NULL.");
    ...
    if (this->m_t_concentration == NULL)
        throw SEIMSException("MUSLE_I30", "CheckInputData", "The time of concentration can not be NULL.");
    ...
    for (int i = 0; i < this->m_cellSize; i++)
    {
        ...
        else
        {
            ...
            if (this->m_snowAccumulation[i] > 0.0001f)
                Y /= exp(3.0f * this->m_snowAccumulation[i] / 25.4f);  //equation 4 in memo, the snow pack effect
...
bool MUSLE_I30::CheckInputSize(const char *key, int n)
{
    if (n <= 0)
    {
        throw SEIMSException("MUSLE_I30", "CheckInputSize",
    ...
    if (this->m_cellSize != n)
    {
        ...
        {
            throw SEIMSException("MUSLE_I30", "CheckInputSize", "Input data for " + string(key) +
...
void MUSLE_I30::SetValue(const char *key, float data)
{
    ...
    if (StringMatch(sk, "cellwidth")) this->m_cellWidth = int(data);
        ...
        throw SEIMSException("MUSLE_I30", "SetValue", "Parameter " + sk
                                                      +
...
void MUSLE_I30::Set1DData(const char *key, int n, float *data)
{
    ...
    if (StringMatch(s, "USLE_C")) this->m_usle_c = data;
        ...
        throw SEIMSException("MUSLE_I30", "SetValue", "Parameter " + s +
                                                      " does not exist in MUSLE_I30 method. Please contact the module developer.");
...
void MUSLE_I30::Get1DData(const char *key, int *n, float **data)
{
    ...
    if (StringMatch(sk, "SOER"))
    {
        *data = this->m_sedimentYield;
        ...
        throw SEIMSException("MUSLE_I30", "getResult", "Result " + sk +
                                                       " does not exist in MUSLE_I30 method. Please contact the module developer.");
