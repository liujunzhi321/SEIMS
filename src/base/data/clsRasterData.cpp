...
float clsRasterData::getAverage(int lyr)
{
	...
	else
	{
		if (lyr < m_nLyrs && m_raster2DData != NULL)
		{
			...
			throw ModelException("clsRasterData", "getAverage","The given layer number is exceed the maximum layers.\n");
	}
}
...
void clsRasterData::outputASCFile(map<string,float> header,int nRows,float** position, float** value,string filename)
{
	...
	int nLyrs = header[HEADER_RS_LAYERS];
...
void clsRasterData::outputGTiff(map<string,float> header,string& srs, int nValidCells,float** position, float* value,string& filename)
{
...
void clsRasterData::outputToMongoDB(map<string,float> header, string &srs, int nValid, float** position, float* value,string remoteFilename, mongoc_gridfs_t* gfs)
{
...
int clsRasterData::ReadFromMongoDB(mongoc_gridfs_t *gfs, const char* remoteFilename)
{
	...
	bson_iter_t iter;
	char* headers[7] = {HEADER_RS_NCOLS, HEADER_RS_NROWS, HEADER_RS_XLL, HEADER_RS_YLL, HEADER_RS_CELLSIZE,HEADER_RS_NODATA,HEADER_RS_LAYERS};
	for (int i = 0; i < 7; i++)
	{
