	...
	for (int iSite = 0; iSite < nSites; iSite++)
	{
		...
		while(mongoc_cursor_more(cursor) && mongoc_cursor_next(cursor,&doc)){
			...
			if (bson_iter_init (&iter, doc) && bson_iter_find (&iter,MONG_HYDRO_DATA_VALUE)){
				...
				throw ModelException("NotRegularMeasurement", "NotRegularMeasurement", "The Value field does not exist in DataValues table.");

			if (bson_iter_init (&iter, doc) && bson_iter_find (&iter,MONG_HYDRO_DATA_UTC)){
				dt = GetDateTimeFromBSONITER(&iter)/1000.;}
			else
				throw ModelException("NotRegularMeasurement", "NotRegularMeasurement", "The UTCDateTime field does not exist in DataValues table.");
