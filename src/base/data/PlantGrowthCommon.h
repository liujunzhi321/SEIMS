...
public:
	...
	static bool IsAnnual(int m_classification) {return 
		m_classification == 1 || 
		m_classification == 2 ||
		m_classification == 4 ||
		m_classification == 5;}
	bool IsLegume(int m_classification) const {return m_classification <= 3;}
	bool IsPerennial(int m_classification) const {return 
		m_classification == 3 ||
		m_classification == 6;}
	bool IsCoolSeasonAnnual(int m_classification) const {return
		m_classification == 2 ||
		m_classification == 5;}
	bool IsGrain(int m_classification) const {return m_classification == 4;}
	bool IsPlant(int luID) const {return luID <= MAX_CROP_LAND_USE_ID && 
