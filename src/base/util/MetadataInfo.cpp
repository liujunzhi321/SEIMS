#include "MetadataInfo.h"
    ...
    {
		int sz = sb->size() + (name.size() + attributes.size() + (indent * 3) + 3);
    ...
    {
		int sz = sb->size() + (name.size() + (indent * 3) + 2);
		if (sz > (int)sb->capacity())
		{
			sb->reserve(sz);
		}
    ...
    {
		int sz = sb->size() + (name.size() + content.size() + (indent * 3) + 5);
		if (sz > (int)sb->capacity())
		{
			sb->reserve(sz);
		}
    ...
    //bool MetadataInfo::WriteXMLFile(string filename)
    //{
	   // bool bStatus;
    //    Stream fs;
    ...
    //        fs = File.OpenWrite(filename);
	   ...
	   // else
	   // {
		  //  ModelFilename fn = GetFile(MFilenameType.FileTypeInfo);
    //        fs = File.OpenWrite(fn.Name);
	   // }
    ...
    //        int noc = encoding.GetByteCount(sb.ToString());
    //        byte[] arr = new byte[noc];
    //        arr = encoding.GetBytes(sb.ToString());
    //        fs.Write(arr, 0, noc);
    //        fs.Flush();
    //        fs.Close();
