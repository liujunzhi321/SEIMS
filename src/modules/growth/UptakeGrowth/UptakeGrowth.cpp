...
UptakeGrowth::UptakeGrowth(void):m_nCells(-1), m_curYear(10.f),//parameter
	 ...
	 m_frN1(NULL), m_frN2(NULL), m_frN3(NULL), m_frP1(NULL), m_frP2(NULL),	m_frP3(NULL), m_maxLAI(NULL),
	m_classification(NULL),	m_NFixFactor(NODATA),	m_NFixDailyMax(NODATA), m_waterDis(NODATA), m_waterCom(NODATA), m_NDis(NODATA), 
	m_PDis(NODATA), m_layers(NODATA), m_rootDepthmax(NULL), m_layerFieldCapacity(NULL), m_layerWiltingPnt(NULL),  m_frDPHU(NULL), m_maxBiomass(NULL), m_fullDevYear(NULL),
	m_zRoot(NULL),m_WnormalizationParam(NULL),m_NnormalizationParam(NULL),m_PnormalizationParam(NULL),m_waterLastDepth(NULL),	
	m_waterThisDepth(NULL), m_waterTotal(NULL),	m_layerAvailableWaterCapacity(NULL),m_potentialNUptake(NULL),m_potentialPUptake(NULL),
	m_NLastDepth(NULL),m_NThisDepth(NULL),m_NTotal(NULL),	m_PLastDepth(NULL),	m_PThisDepth(NULL),m_PTotal(NULL),	
	m_fGrowthStage(NULL),m_fSoilWater(NULL),m_fSoilNitrate(NULL),m_totalSoilWater(NULL), m_totalNO3(NULL), m_totalFieldCapacity(NULL),
	//input
	m_LAIdelta(NULL), m_LAI(NULL), m_preLAI(NULL), m_frPHU(NULL), m_frN(NULL), m_frP(NULL), m_frRoot(NULL),
	m_biomassDelta(NULL), m_biomass(NULL), m_biomassNOpt(NULL), m_biomassPOpt(NULL),
	m_layerNO3(NULL), m_layerP(NULL), m_tMax(NULL), m_tMin(NULL),m_PET(NULL), m_layerSoilWater(NULL),
	
	//result
	m_bioTreeAnnual(NULL),m_biomassN(NULL), m_biomassP(NULL), m_biomassRoot(NULL), m_biomassAG(NULL), m_transpirationMax(NULL), m_depthArray(NULL), 
	m_layerWUptake(NULL),	 m_layerNUptake(NULL), m_layerPUptake(NULL), m_NFixation(NULL), m_totalWUptake(NULL), m_totalNUptake(NULL), m_totalPUptake(NULL),//m_aerationStress(NULL), 
	m_waterStress(NULL), m_tempStress(NULL), m_NStress(NULL), m_PStress(NULL), m_totalStress(NULL)
{
...
bool UptakeGrowth::CheckInputData(void)
{
	...
	if(FloatEqual(m_NDis, NODATA))			
		throw ModelException("UptakeGrowth", "CheckInputData","The nitrogen uptake distribution parameter can not be -99.");
	if(FloatEqual(m_PDis, NODATA))			
		throw ModelException("UptakeGrowth", "CheckInputData","The phosphorus uptake distribution parameter can not be -99.");
	...
	if(m_rootDepthmax == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The maximum depth for root development in the soil data can not be NULL.");
	...
	if(m_layerWiltingPnt == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The Plant wilting point moisture data can not be NULL.");
	...
	if(m_layerP == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The amount of phosphorus in solution in layer ly can not be NULL.");
	...
	if(m_frDPHU == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The PHU fraction when senescence becomes dominant data can not be NULL.");
	...
	if(m_frN1 == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The normal fraction of nitrogen in the plant biomass at emergence data can not be NULL.");
	if(m_frN2 == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The normal fraction of nitrogen in the plant biomass at 50% maturity data can not be NULL.");
	if(m_frN3 == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The normal fraction of nitrogen in the plant biomass at maturity data can not be NULL.");
	if(m_frP1 == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The normal fraction of phosphorus in the plant biomass at emergence data can not be NULL.");
	if(m_frP2 == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The normal fraction of phosphorus in the plant biomass at 50% maturity data can not be NULL.");
	if(m_frP3 == NULL)			
		throw ModelException("UptakeGrowth","CheckInputData","The normal fraction of phosphorus in the plant biomass at maturity data can not be NULL.");
	if(m_maxLAI == NULL)			
	...
	if(m_PET == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The Potential evapotranspiration data can not be NULL.");

	if(m_LAIdelta == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The potential leaf area added on day data can not be NULL.");
	if(m_LAI == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The leaf area index for a given day data can not be NULL.");
	if(m_preLAI == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The leaf area index for previous day data can not be NULL.");

	
	
	if(m_biomass == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The total biomass on a given day data can not be NULL.");
	if(m_biomassDelta == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The biomass increase on a given day data can not be NULL.");
	...
	if(m_frPHU == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The fraction of phu accumulated on a given day data can not be NULL.");
	if(m_frN == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The optimal fraction of nitrogen in the plant biomass data can not be NULL.");
	if(m_frP == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The optimal fraction of phosphorus in the plant biomass data can not be NULL.");
	if(m_frRoot == NULL)			
		throw ModelException("UptakeGrowth", "CheckInputData","The fraction of total biomass partitioned to roots on a given day data can not be NULL.");
...
bool UptakeGrowth::CheckInputSize(const char* key, int n)
{
	if(n<=0)
	{
		throw ModelException("UptakeGrowth","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero00.");
		return false;
	}
	if(m_nCells != n)
	{
		...
		{
			throw ModelException("UptakeGrowth","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			return false;
		}
	}
	return true;
...
void UptakeGrowth::SetValue(const char* key, float data)
{
	...
	else									throw ModelException("UptakeGrowth","SetValue","Parameter " + s 
...
void UptakeGrowth::Get1DData(const char* key, int* n, float** data)
{
	...
	else if (StringMatch(s, "N_Fixation"))
	{
		*data = m_NFixation;
		...
		throw ModelException("UptakeGrowth","Get1DData","Result " + s + " does not exist in UptakeGrowth method. Please contact the module developer.");
...
void UptakeGrowth::Get2DData(const char* key, int* nRows, int* nCols, float*** data)
{
	...
	else if (StringMatch(s, "Layer_PUptake"))
	{
		...
		throw ModelException("UptakeGrowth","Get2DData","Result " + s + " does not exist in UptakeGrowth method. Please contact the module developer.");
	...
	if (m_date >= startDate && m_date <= endDate)
	{
		...
		for (int i = 0; i < m_nCells; i++) 
		{
			...
			for (int j = 0; j < m_layers; j++)
			{
				...
				m_waterThisDepth[i] = m_transpirationMax[i] * (1.0f - exp(-m_waterDis * m_depthArray[j+1] / m_zRoot[i])) / m_WnormalizationParam[i];
				...
				{
					m_layerWUptake[i][j] *= exp(5.0f * (4.0f * m_layerSoilWater[i][j] / m_layerAvailableWaterCapacity[i][j] - 1.0f));
				}
			...
			for (int j = 0; j<m_layers; j++)
			{
				m_NThisDepth[i] = m_potentialNUptake[i] * (1.0f - exp(-m_NDis * m_depthArray[j+1] / m_zRoot[i])) / m_NnormalizationParam[i];
				m_PThisDepth[i] = m_potentialPUptake[i] * (1.0f - exp(-m_PDis * m_depthArray[j+1] / m_zRoot[i])) / m_PnormalizationParam[i];
		   ...
		   //******************************************************************//
			...
			for (int j = 0; j<m_layers; j++)
			{
				...
				m_fSoilWater[i] = WaterFactorForNitrogenFixation(m_totalSoilWater[i], m_totalFieldCapacity[i]);//////must be total field capacity
			...
			if(m_common->IsLegume())
			{
				...
				fixn = min(fixn, m_NFixDailyMax);					//nitrogen fixation can't surpass maximum limitation
				m_NFixation[i] = fixn;
