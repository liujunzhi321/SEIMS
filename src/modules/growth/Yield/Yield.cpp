...
Yield::Yield(void):m_nCells(-1),
    ...
    m_clip(NULL), m_clipN(NULL), m_clipP(NULL), m_removeFraction(NULL), m_rootFraction(NULL), m_rootLeft(NULL), m_rootRemove(NULL), 
...
bool Yield::CheckInputData(void)
{
	...
	if(m_HiOpt == NULL)			
		throw ModelException("Yield","CheckInputData","The potential harvest index for the plant at maturity given ideal growing conditions data can not be NULL.");
	if(m_HiMin == NULL)			
		throw ModelException("Yield","CheckInputData","The harvest index for the plant in drought conditions and represents the minimum harvest index allowed for the plant data can not be NULL.");
	if(m_frNyld == NULL)			
		throw ModelException("Yield","CheckInputData","The fraction of nitrogen in the yield data can not be NULL.");
	if(m_frPyld== NULL)			
		throw ModelException("Yield","CheckInputData","The fraction of phosphorus in the yield data can not be NULL.");
	if(m_frDeclineLAI== NULL)			
		throw ModelException("Yield","CheckInputData","The Decline LAI Fraction data can not be NULL.");

	if(m_frPHU == NULL)			
		throw ModelException("Yield","CheckInputData","The fraction of phu accumulated on a given day data can not be NULL.");
	if(m_PET == NULL)			
	...
	if(m_biomassAG == NULL)		
		throw ModelException("Yield","CheckInputData","The aboveground biomass on the day of harvest data can not be NULL.");
...
bool Yield::CheckInputSize(const char* key, int n)
{
	if(n<=0)
	{
		throw ModelException("Yield","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(m_nCells != n)
	{
		...
		{
			throw ModelException("Yield","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			return false;
		}
	}
	return true;
...
void Yield::Get1DData(const char* key, int* n, float** data)
{
	...
	else if (StringMatch(s, "Yield"))
	{
		*data = m_yield;
		...
		throw ModelException("Yield","Get1DData","Result " + s + " does not exist in Yield method. Please contact the module developer.");
	...
	for (int i = 0; i < m_nCells; i++) 
	{
		if (m_date > harvestDate && !m_harvested)
		{
			...
			else //biomass harvest, has residue, residue redistribute to soil layer, see harvestop.f
			{
				...
				m_clipN[i] = max(0.0f,min(m_clip[i] * m_frNyld[i], m_biomassN[i] - m_yieldN[i])); //N in clip residual,needed by nitrient cycling module
				m_clipP[i] = max(0.0f,min(m_clip[i] * m_frPyld[i], m_biomassP[i] - m_yieldP[i])); //P in clip residual,needed by nitrient cycling module
				...
				m_rootLeft[i] = (m_biomass[i] - m_biomassRoot[i]) * (1-m_removeFraction[i]) * m_rootFraction[i] / (1.0f - m_rootFraction[i]);
