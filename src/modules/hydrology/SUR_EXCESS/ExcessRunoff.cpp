...
ExcessRunoff::ExcessRunoff(void):m_infil(NULL),m_pe(NULL), m_accumuDepth(NULL), m_infilCapacitySurplus(NULL),
	m_dt(-1), m_nCells(-1), m_fieldCap(NULL), m_porosity(NULL), m_ks(NULL), m_rootDepth(NULL), m_initSoilMoisture(NULL),
...
void ExcessRunoff::Get1DData(const char* key, int* n, float **data)
{
	*n = m_nCells;
	...
	{
		throw ModelException("SUR_EXCESS","Get1DData","Parameter " + sk + " does not exist. Please contact the module developer.");
...
bool ExcessRunoff::CheckInputSize(const char* key,int n)
{
	if(n<=0)
	{
		throw ModelException("SUR_EXCESS","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(this->m_nCells != n)
	{
		...
		{
			throw ModelException("SUR_EXCESS","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			ostringstream oss;
...
void ExcessRunoff::Set1DData(const char* key, int n, float* data)
{
	...
	{
		throw ModelException("SUR_EXCESS","SetValue","Parameter " + sk + " does not exist. Please contact the module developer.");
	}
}
