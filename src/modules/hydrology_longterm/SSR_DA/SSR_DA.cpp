...
SSR_DA::SSR_DA(void) : m_nSoilLayers(2), m_dt(-1), m_nCells(-1), m_CellWidth(-1), m_nSubbasin(-1), m_frozenT(-99.f),
                       m_ki(-99.f), m_ks(NULL),
                       m_porosity(NULL), m_poreIndex(NULL), m_sm(NULL), m_fc(NULL), m_soilT(NULL), m_rootDepth(NULL),
                       m_slope(NULL), m_qi(NULL), m_qiVol(NULL),
                       m_chWidth(NULL), m_streamLink(NULL), m_flowInIndex(NULL), m_flowInPercentage(NULL),
...
void SSR_DA::SetValue(const char *key, float data)
{
    ...
    if (StringMatch(s, VAR_OMP_THREADNUM))
        ...
        throw ModelException("SSR_DA", "SetValue",
                             "Parameter " + s +
                             " does not exist in SSR_DA method. Please contact the module developer.");
}
...
bool SSR_DA::CheckInputSize(const char *key, int n)
{
    if (n <= 0)
    {
        throw ModelException("SSR_DA", "CheckInputSize",
    ...
    if (this->m_nCells != n)
    {
        ...
        {
            throw ModelException("SSR_DA", "CheckInputSize", "Input data for " + string(key) +
