...
void ExcessRunoff::Get1DData(const char* key, int* n, float **data)
{
	*n = m_nCells;
	...
	{
		throw ModelException("SUR_ES","Get1DData","Parameter " + sk + " does not exist. Please contact the module developer.");
...
bool ExcessRunoff::CheckInputSize(const char* key,int n)
{
	if(n<=0)
	{
		throw ModelException("SUR_ES","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(this->m_nCells != n)
	{
		...
		{
			throw ModelException("SUR_ES","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			ostringstream oss;
...
void ExcessRunoff::Set1DData(const char* key, int n, float* data)
{
	...
	{
		throw ModelException("SUR_ES","SetValue","Parameter " + sk + " does not exist. Please contact the module developer.");
	}
}
