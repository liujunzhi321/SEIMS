...
void IUH_IF::SetValue(const char* key, float value)
{
	...
	else									throw ModelException("IUH_IF","SetValue","Parameter " + sk 
...
void IUH_IF::Get1DData(const char* key, int* n, float** data)
{
	...
	else									throw ModelException("IUH_IF","getResult","Result " + sk + " does not exist in IUH_IF method. Please contact the module developer.");
