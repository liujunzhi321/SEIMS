...
bool PETPenmanMonteith::CheckInputData()
{
	...
	{
		throw ModelException(MID_PET_PM,"CheckInputData","The vapor pressure deficit(kPa) corresponding to the second point on the stomatal conductance curve can not be NULL.");
		return false;
	}
	if(this->m_frgmax == NULL)
	{
		throw ModelException(MID_PET_PM,"CheckInputData","The fraction of maximum stomatal conductance can not be NULL.");
		return false;
	}
	return true;
}
