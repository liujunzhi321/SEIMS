...
{
	if(m_nCells <= 0)				throw ModelException(MID_SSM_PE,"CheckInputData","The dimension of the input data can not be less than zero.");
	if(this->m_kblow == NODATA)		throw ModelException(MID_SSM_PE,"CheckInputData","The fraction coefficient of snow blowing into or out of the watershed can not be NULL.");
	if(this->m_Pnet == NULL)		throw ModelException(MID_SSM_PE,"CheckInputData","The net precipitation data can not be NULL.");
	if(this->m_PET == NULL)			throw ModelException(MID_SSM_PE,"CheckInputData","The PET data can not be NULL.");
	if(this->m_SA == NULL)			throw ModelException(MID_SSM_PE,"CheckInputData","The snow accumulation data can not be NULL.");
	...
	if(this->m_tMean == NULL)		throw ModelException(MID_SSM_PE,"CheckInputData","The mean air temperature data can not be NULL.");
	if(this->m_t0 == NODATA)			throw ModelException(MID_SSM_PE,"CheckInputData","The Snowmelt temperature can not be NODATA.");
	if(this->m_tsnow == NODATA)		throw ModelException(MID_SSM_PE,"CheckInputData","The snow fall temperature can not be NODATA.");
	return true;
}
...
bool SSM_PE::CheckInputSize(const char* key, int n)
{
	if(n<=0)
	{
		throw ModelException(MID_SSM_PE,"CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(this->m_nCells != n)
	{
		...
		{
			throw ModelException(MID_SSM_PE,"CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			return false;
		}
	}
	return true;
...
void SSM_PE::Set1DData(const char* key, int n, float* data)
{
	...
	else									throw ModelException(MID_SSM_PE,"Set1DData","Parameter " + s + 
		" does not exist in current module. Please contact the module developer.");
}
