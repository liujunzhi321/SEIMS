...
bool SRD_MB::CheckInputData(void)
{
	...
	if(this->m_kblow == -99)		throw ModelException("SRD_MB","CheckInputData","The fraction coefficient of snow blowing into or out of the watershed can not be NULL.");
	if(this->m_Pnet == NULL)		throw ModelException("SRD_MB","CheckInputData","The net precipitation data can not be NULL.");
	if(this->m_curva_wind == NULL)	throw ModelException("SRD_MB","CheckInputData","The curvature wind data can not be NULL.");
	if(this->m_slope_wind == NULL)	throw ModelException("SRD_MB","CheckInputData","The slope wind data can not be NULL.");
	if(this->m_shc == NULL)			throw ModelException("SRD_MB","CheckInputData","The snow hold capacity data can not be NULL.");
	if(this->m_SA == NULL)			throw ModelException("SRD_MB","CheckInputData","The snow hold capacity data can not be NULL.");
	...
	if(this->m_wsSize <=0)		throw ModelException("SRD_MB","CheckInputData","The dimension of the wind speed data can not be less than zero.");
...
int SRD_MB::Execute()
{
	...
	for ( int rw = 0; rw < this->m_cellSize; rw++)
	{
		...
		if(snow < 0.01)			// if snow is lower than a very small positive value (0.01), consider it is equal to 0.
		{
			this->m_SR[rw] = 0.0f;
			continue;
		}
...
void SRD_MB::SetValue(const char* key, float data)
{
	...
	else									throw ModelException("SRD_MB","SetValue","Parameter " + s + 
