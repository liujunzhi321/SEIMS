...
bool SNO_WB::CheckInputData(void)
{
	...
	if(this->m_nCells <= 0)				throw ModelException(MID_SNO_WB,"CheckInputData","The dimension of the input data can not be less than zero.");
	if(this->m_P == NULL)			throw ModelException(MID_SNO_WB,"CheckInputData","The precipitation data can not be NULL.");
	if(this->m_tMean == NULL)		throw ModelException(MID_SNO_WB,"CheckInputData","The mean temperature data can not be NULL.");
	if(this->m_tMax == NULL)		throw ModelException(MID_SNO_WB,"CheckInputData","The max temperature data can not be NULL.");
	if(this->m_WindSpeed == NULL)	throw ModelException(MID_SNO_WB,"CheckInputData","The wind speed data can not be NULL.");
	if(this->m_kblow == NODATA)		throw ModelException(MID_SNO_WB,"CheckInputData","The fraction coefficient of snow blowing into or out of the watershed can not be NULL.");
	if(this->m_t0 == NODATA)			throw ModelException(MID_SNO_WB,"CheckInputData","The snowmelt threshold temperature can not be NULL.");
	if(this->m_tsnow == NODATA)		throw ModelException(MID_SNO_WB,"CheckInputData","The snowfall threshold temperature can not be NULL.");
	if(this->m_swe0 == NODATA)			throw ModelException(MID_SNO_WB,"CheckInputData","The Initial snow water equivalent can not be NULL.");
	...
	if(this->m_Pnet == NULL)		throw ModelException(MID_SNO_WB,"CheckInputData","The net precipitation data can not be NULL.");
	...
	if(this->m_SE == NULL)			throw ModelException(MID_SNO_WB,"CheckInputData","The snow sublimation data can not be NULL.");
	return true;
}

void SNO_WB::initalOutputs()
{
	if(m_nCells <= 0)				throw ModelException(MID_SNO_WB,"CheckInputData","The dimension of the input data can not be less than zero.");
...
int SNO_WB::Execute()
{
	...
	for ( int rw = 0; rw < this->m_nCells; rw++)
	{
		...
		else if(dT > this->m_tsnow && dT < this->m_t0 && dSA > dPnet) //rain on snow, if the snow accumulation is larger than net precipitation, assume all the precipitation is accumulated in snow pack.
		{
			dtmp2 += dPnet;
		}
...
bool SNO_WB::CheckInputSize(const char* key, int n)
{
	if(n<=0)
	{
		throw ModelException(MID_SNO_WB,"CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(this->m_nCells != n)
	{
		...
		{
			throw ModelException(MID_SNO_WB,"CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			return false;
		}
	}
	return true;
...
void SNO_WB::SetValue(const char* key, float data)
{
	...
	else									throw ModelException(MID_SNO_WB,"SetValue","Parameter " + s 
...
void SNO_WB::Get1DData(const char* key, int* n, float** data)
{
	...
	{		
		...
		*n = this->m_nCells;
		...
		throw ModelException(MID_SNO_WB,"Get1DData","Result " + s + " does not exist in current module. Please contact the module developer.");
}
...
void SNO_WB::GetValue(const char* key, float* data)
{
	...
	else									throw ModelException(MID_SNO_WB,"GetValue","Result " + s + 
		" does not exist in current module. Please contact the module developer.");	
}
