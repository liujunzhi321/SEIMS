...
bool IUH_OL::CheckInputData(void)
{
    if (m_nCells < 0)
    {
        throw ModelException("IUH_OL", "CheckInputData", "The parameter: m_nCells has not been set.");
        return false;
    }
...
bool IUH_OL::CheckInputSize(const char *key, int n)
{
    if (n <= 0)
    {
        throw ModelException("IUH_OL", "CheckInputSize",
    ...
    if (this->m_nCells != n)
    {
        ...
        {
            throw ModelException("IUH_OL", "CheckInputSize", "Input data for " + string(key) +
...
void IUH_OL::SetValue(const char *key, float value)
{
    ...
    else if (StringMatch(sk, VAR_OMP_THREADNUM))
    {
        ...
        throw ModelException("IUH_OL", "SetValue", "Parameter " + sk
                                                   +
