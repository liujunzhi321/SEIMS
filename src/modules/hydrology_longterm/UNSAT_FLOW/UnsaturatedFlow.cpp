...
{
	if(m_cellSize <= 0)					throw ModelException("UnsaturatedFlow","CheckInputData","The dimension of the input data can not be less than zero.");

	if(this->m_FieldCap == NULL)		throw ModelException("UnsaturatedFlow","CheckInputData","The Soil field capacity can not be NULL.");
	if(this->m_WiltPoint == NULL)		throw ModelException("UnsaturatedFlow","CheckInputData","The plant wilting point moisture can not be NULL.");
	...
	if(this->m_Moist == NULL)			throw ModelException("UnsaturatedFlow","CheckInputData","The soil moisture can not be NULL.");
	if(this->m_SoilT == NULL)			throw ModelException("PER_PI","CheckInputData","The soil temerature can not be NULL.");
	if(this->m_ForzenT == -99.0f)		throw ModelException("PER_PI","CheckInputData","The threshold soil freezing temerature can not be NULL.");

	if(this->m_percolation == NULL)		throw ModelException("SSR_DA","CheckInputData","The percolation can not be NULL.");
	if(this->m_infiltration == NULL)	throw ModelException("SSR_DA","CheckInputData","The infiltration can not be NULL.");
...
bool UnsaturatedFlow::CheckInputSize(const char* key, int n)
{
	if(n<=0)
	{
		throw ModelException("UnsaturatedFlow","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(this->m_cellSize != n)
	{
		...
		{
			throw ModelException("UnsaturatedFlow","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			return false;
		}
	}
