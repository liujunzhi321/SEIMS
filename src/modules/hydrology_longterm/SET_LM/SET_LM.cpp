...
SET_LM::SET_LM(void):m_nSoilLayers(2), m_nCells(-1), m_fc(NULL), m_wp(NULL), m_EI(NULL), m_PET(NULL), m_ED(NULL), m_sm(NULL), m_soilT(NULL),
...
{
	if(n<=0)
		throw ModelException("SET_LM","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
	...
	if(m_nCells != n)
	{
		if(m_nCells <=0) 
			...
			throw ModelException("SET_LM","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
	}
