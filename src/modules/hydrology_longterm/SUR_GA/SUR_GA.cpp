...
SUR_GA::SUR_GA(void):m_TimeStep(NODATA),m_Conductivity(NULL),m_porosity(NULL),m_clay(NULL),m_sand(NULL),m_rootDepth(NULL),
	m_CN2(NULL),m_P_NET(NULL),m_cellSize(-1),m_fieldCap(NULL),m_wiltingPoint(NULL),m_soilMoisture(NULL),m_INFRate(NULL),
	m_SD(NULL),m_tMin(NULL),m_tMax(NULL),m_Tsnow(NODATA),m_Tsoil(NODATA),m_Sfrozen(NODATA),m_T0(NODATA),m_SM(NULL),
	m_SA(NULL),m_TS(NULL),m_mask(NULL),m_INFIL(NULL),m_PE(NULL),m_date(-1),m_w1(NULL),m_w2(NULL),m_sMax(NULL),m_wfmp(NULL)
{
...
bool SUR_GA::CheckInputData(void)
{
	...
	{
		throw ModelException("SUR_GA","CheckInputData","The saturated hydraulic conductivity of the input data can not be NULL.");
		return false;
	}
	...
	if (this->m_clay == NULL)
	{
		throw ModelException("SUR_GA","CheckInputData","The percent of clay content of the input data can not be NULL.");
		return false;
	}

	if (this->m_sand == NULL)
	{
		throw ModelException("SUR_GA","CheckInputData","The percent of sand content of the input data can not be NULL.");
		return false;
	}
	...
	if (this->m_CN2 == NULL)
	{
		throw ModelException("SUR_GA","CheckInputData","The CN under moisture condition II of the input data can not be NULL.");
		return false;
	}
	...
	{
		throw ModelException("SUR_GA","CheckInputData","The water content of soil at field capacity of the input data can not be NULL.");
		return false;
	}

	if (this->m_wiltingPoint == NULL)
	{
		throw ModelException("SUR_GA","CheckInputData","The plant wilting point moisture of the input data can not be NULL.");
		return false;
	}
	...
	if (this->m_SD == NULL )
	{
		throw ModelException("SUR_GA","CheckInputData","The depression storage or the depression storage capacity and depression storage coefficient of the input data can not be NULL.");
		return false;
	}
	...
	{
		throw ModelException("SUR_GA","CheckInputData","The soil freezing temperature of the input data can not be NULL.");
		return false;
	}

	if (FloatEqual(this->m_T0, NODATA))
	{
		throw ModelException("SUR_GA","CheckInputData","The snowmelt threshold temperature of the input data can not be NULL.");
		return false;
	}
...
{
	if(m_cellSize <= 0)				throw ModelException("SUR_GA","CheckInputData","The dimension of the input data can not be less than zero.");
...
int SUR_GA::Execute()
{
	...
	for(int iCell=0; iCell < m_cellSize; iCell++)
	{
		...
		if (m_INFIL[iCell] > 10000.0f || m_INFIL[iCell] < 0.0f)
		{
			...
			ostringstream oss;
			oss << "Cell(Row:" << m_mask[iCell][0] << ", Col:" << m_mask[iCell][1]<<"\n Infiltration =" << m_INFIL[iCell]
			<< "\n Precipitation(mm) = " << pNet << "\n InfiltrationRate(m/h) = " << rateinf << "\n";
			throw ModelException("SUR_GA","Execute","Output data error: infiltration is out of reasonable range. Where:\n"
...
bool SUR_GA::CheckInputSize(const char* key, int n)
{
	if(n<=0)
	{
		throw ModelException("SUR_GA","CheckInputSize","Input data for "+string(key) +" is invalid. The size could not be less than zero.");
		return false;
	}
	if(this->m_cellSize != n)
	{
		...
		{
			throw ModelException("SUR_GA","CheckInputSize","Input data for "+string(key) +" is invalid. All the input data should have same size.");
			return false;
		}
	}
...
void SUR_GA::SetValue(const char* key, float value)
{
	...
	else									throw ModelException("SUR_GA","SetValue","Parameter " + sk 
...
void SUR_GA::Get1DData(const char* key, int* n, float** data)
{
	...
	else									throw ModelException("SUR_GA","getResult","Result " + sk + " does not exist in SUR_GA method. Please contact the module developer.");
...
float SUR_GA::Calculate_WFMP(float sol_por, float sol_clay, float sand) //this function calculated the wetting front matric potential
{
	float wfmp = 10.0f * exp(6.5309f - 7.32561f * sol_por + 3.809479f * pow(sol_por, 2) + 0.001583f *
		pow(sol_clay, 2) + 0.000344f * sand * sol_clay - 0.049837f * sol_por * sand + 0.001608f * 
		pow(sol_por, 2) * pow(sand, 2) + 0.001602f * pow(sol_por, 2) * pow(sol_clay, 2) - 
