...
SUR_MR::SUR_MR(void) : m_nCells(-1), m_tSnow(NODATA), m_t0(NODATA), m_tFrozen(NODATA), m_kRunoff(NODATA),
                       m_pMax(NODATA),
                       m_sFrozen(NULL), m_runoffCo(NULL), m_initSoilMoisture(NULL), m_soilDepth(NULL), m_fieldCap(NULL),
                       m_porosity(NULL),
...
void SUR_MR::CheckInputData(void)
{
    ...
    if (FloatEqual(m_pMax, NODATA))
        throw ModelException(MID_SUR_MR, "CheckInputData",
                             "The maximum P corresponding to runoffCo of the input data can not be NULL.");
