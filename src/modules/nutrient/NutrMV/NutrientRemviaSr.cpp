...
void NutrientRemviaSr::Nitrateloss(){
	...
	for(int i = 0; i < m_nCells; i++) {
		for(int k = 0; k < m_nSoilLayers[i]; k++) {
			...
			if (m_ldrain[i] == k) {
					// m_alph_e[i] = exp(-1.f/(m_n_lag[i] + 1.e-6f))
					// ww1 = -1.f/ ((1.f - m_anion_excl[i]) * m_sol_wsatur[i])
					// m_vno3_c = m_sol_no3[i][k] * (1.f - exp(ww1))
					// if (total_no3 > 1.001f) {
					//	 tno3ln = n_lnco[i] * (log(total_no3)) ** m_n_ln[i]
					// else
					//	 tno3ln = 0.f
					// }
					// vno3_c = tno3ln * (1.f - Exp(ww1))
					// co_p[i] = co_p[i] * (1.f - alph_e[i]) + vno3_c * alph_e[i]
		...
		if (qdr > 1.e-4f) {
			...
			float yy = 0.f;
			float zz = 0.f;
			ww = -139.34410f + (1.575701E05 / wtmp);
			xx = 6.642308E07f /pow (wtmp, 2);
			yy = 1.243800E10f /pow (wtmp, 3);
			zz = 8.621949E11f / pow(wtmp, 4);
			m_soxy[i] = exp(ww - xx + yy - zz);
			if (soxy < 0.f) {
				soxy = 0.f;
			}
			// calculate actual dissolved oxygen concentration
			m_doxq[i] = soxy * exp(-0.1f * cbod);
			if (m_doxq[i] < 0.f) {
				m_doxq[i] = 0.f;
			}
			if (m_doxq[i] > soxy) {
