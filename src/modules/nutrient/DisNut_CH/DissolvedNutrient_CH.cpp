...
bool DissolvedNutrient_CH::CheckInputData(void)
{
	...
	if(m_flowInIndex == NULL)
		throw ModelException("DissolvedNutrient_CH","CheckInputData","The parameter: flow in index has not been set.");
	...
	if(m_ChannelWH == NULL)
		throw ModelException("DissolvedNutrient_CH","CheckInputData","The parameter: Channel water hight has not been set.");
	if(m_ChQkin == NULL)
		throw ModelException("DissolvedNutrient_CH","CheckInputData","The parameter: Channel water flow has not been set.");
	if(m_DissovPToCh == NULL)
		throw ModelException("DissolvedNutrient_CH","CheckInputData","The parameter: m_DissovPToCh has not been set.");
	if(m_AmmoniumToCh == NULL)
		throw ModelException("DissolvedNutrient_CH","CheckInputData","The parameter: m_AmmoniumToCh has not been set.");
	if(m_NitrateToCh == NULL)
		throw ModelException("DissolvedNutrient_CH","CheckInputData","The parameter: m_NitrateToCh has not been set.");
...
void DissolvedNutrient_CH::GetNutrientInFlow(int iReach, int iCell, int id)
{
	...
	m_DissovP[iReach][iCell] = simpleSedCalc(m_ChQkin[iReach][iCell], Qin, PUp, m_dt, WtVol, m_DissovP_KG[iReach][iCell]);
	m_Ammonium[iReach][iCell] = simpleSedCalc(m_ChQkin[iReach][iCell], Qin, NH4Up, m_dt, WtVol, m_Ammonium_KG[iReach][iCell]);
	m_Nitrate[iReach][iCell] = simpleSedCalc(m_ChQkin[iReach][iCell], Qin, NO3Up, m_dt, WtVol, m_Nitrate_KG[iReach][iCell]);
