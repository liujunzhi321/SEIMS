...
bool KinWavSed_OL::CheckInputData()
{
	...
	if(m_Ccoe < 0)
	{
		throw ModelException("Soil_DET","CheckInputData","You have not set the calibration coefficient of overland erosion.");
		return false;
	}
	if (m_USLE_K == NULL)
	{
...
{
	float q, S0, K;
	q = m_V[id] * m_WH[id]*60;   // m2/s -> m2/min                                 // m_Qkin[id]*60;   // convert to m3/min
...
float KinWavSed_OL::complexSedCalc(float Qj1i1, float Qj1i, float Qji1,float Sj1i, float Sji1, float alpha, float dt,float dx)
{
