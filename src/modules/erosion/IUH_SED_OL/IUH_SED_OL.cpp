...
IUH_SED_OL::IUH_SED_OL(void) : m_TimeStep(-1), m_nCells(-1), m_CellWidth(NODATA_VALUE), m_cellArea(NODATA_VALUE),
					   ...
					   m_sedYield(NULL), m_cellSed(NULL), m_sedtoCh(NULL)
{
}
...
bool IUH_SED_OL::CheckInputData(void)
{
    ...
    if (m_rs == NULL)
        throw ModelException(MID_IUH_SED_OL, "CheckInputData", "The parameter: surface runoff (m_rs) has not been set.");
    if (m_date < 0)
    ...
    for (int i = 0; i < m_nCells; i++)
    {
        ...
        if (v_rs > 0.f)
        {
            ...
            for (int k = min; k <= max; k++)
            {
				...
				// calculate the sediment routing if the input sediment yield is available
				if(m_sedYield != NULL)
