...
void ModulesIOList::ReadDLL(string& id, string& dllID)
{
	...
	utils util;
	if(!util.FileExists(moduleFileName)) throw ModelException("ModulesIOList", "ReadDLL", moduleFileName + " does not exist or has no read permission!");
	...
	if(m_instanceFuncs[id] == NULL) 
		throw ModelException("ModulesIOList", "ReadDLL", moduleFileName + " does not implement API function: GetInstance");
	if(m_metadataFuncs[id] == NULL) 
		throw ModelException("ModulesIOList", "ReadDLL", moduleFileName + " does not implement API function: MetadataInformation");
...
void ModulesIOList::ReadParameterSetting(string& moduleID, TiXmlDocument& doc, SEIMSModuleSetting* setting)
{
	...
	if (eleParams != NULL)
	{
		...
		while (eleParam != NULL)
		{
			...
			{
				delete param;
				throw ModelException("ModulesIOList", "ReadParameterSetting", "Some parameters have not name in metadata!");
			}
			...
			if (param->Source.size() <= 0)
			{
				...
				delete param;
				throw ModelException("ModulesIOList", "ReadParameterSetting", "parameter "+ name + " does not have source!");
			}
			...
			if (param->Dimension == DT_Unknown)
			{
				string name = param->Name;
				delete param;
				throw ModelException("ModulesIOList", "ReadParameterSetting", "parameter "+ name + " does not have dimension!");
			}
