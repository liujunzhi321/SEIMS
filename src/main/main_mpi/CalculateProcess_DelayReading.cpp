	...
	string& projectPath, string& modulePath, const char* host, int port , const char* dbName, int nThreads, LayeringMethod layeringMethod)
{
	...
	{
        //cout << rank << " " <<  pTasks[i] << " " << dbName << endl;
		WetSpaMain *p = new WetSpaMain(conn, db, projectPath, factory, pTasks[i], 0, layeringMethod);
        if(i == 0)
        {
	...
	//{
		...
		//cout << "[DEBUG][TIMESPAN][INPUT]\t"  << numprocs-1 << "\t" << ioTime << "\n";
	//}
	//t1 = MPI_Wtime();
	...
	for(time_t t = p->getStartTime(); t <= p->getEndTime();t += dtCh)
	{
		...
		for (size_t j = 0; j < sourceBasins.size(); ++j)
		{
            ...
            WetSpaMain *pSubbasin = modelList[index];
			...
			//if (rank == R)
			//	cout << rank << ":" << pTasks[index] << " Before execution" << endl;
			//modelList[index]->Step(t);
			for(int i = 0; i < nHs; ++i)
				pSubbasin->StepHillSlope(t+i*dtHs, i);
			//if (rank == R)
			//	cout << rank << ":" << pTasks[index] << "End Hillslope execution" << endl;
			pSubbasin->StepChannel(t);
            pSubbasin->Output(t);
			//if (rank == R)
			//	cout << rank << ":" << pTasks[index] << "End execution" << endl;
	...
	if (slaveRank == 0)
	{
		...
		cout << "[DEBUG][TIMESPAN][COMPUTING]\t" << numprocs-1 << "\t" << computingTime << "\n";
	...
	if (slaveRank == 0)
	{
		...
		cout << "[DEBUG][TIMESPAN][OUTPUT]\t" << numprocs-1 << "\t" << outputTime << "\n";
	}
	...
	if (slaveRank == 0)
	{
		...
		cout << "[DEBUG][TIMESPAN][ALL]\t" << numprocs-1 << "\t" << allTime << "\n";
	}
