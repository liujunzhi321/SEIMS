	...
	string& projectPath, string& modulePath, const char* host, int port , const char* dbName, int nThreads, LayeringMethod layeringMethod)
{
	...
	if(rank == SLAVE0_RANK)
	{
		...
		cout << "slave0:\t" << rank << "\t" << SLAVE0_RANK << endl;
	}
...
//#endif
	...
	SettingsInput *input = new SettingsInput(projectPath + File_Input, conn, string(dbName), -1);	
	ModuleFactory *factory = new ModuleFactory(projectPath + File_Config, modulePath, conn, string(dbName));
	
	//cout << "before set up model.\n";
	for (int i = 0; i < nSubbasins; i++)
	{
		//cout << "set up model object: " << rank << "  subbasin:" << pTasks[i] << endl;
	...
	//cout << "set up model object: " << rank << endl;
	...
	//if (slaveRank == 0)
	//cout << "Time of reading data -- Max:" << Max(tReceive, nSlaves) << "   Total:" << Sum(tReceive, nSlaves) << "\n";
	...
	utils util;
	//time_t st = p->getStartTime();
	//time_t et = p->getEndTime();
	//cout << util.ConvertToString2(&st) << "\t" << util.ConvertToString2(&et) << endl;
	...
	if (slaveRank == 0)
	{
		cout << "nprocs: " << nSlaves << " Max: " << Max(tReceive, nSlaves) << "   Total: " << Sum(tReceive, nSlaves) << "\n";
	}
	...
	//cout << "Rank: " << slaveRank << " Time of Slope process: " << tSlope << endl;
	//cout << "Rank: " << slaveRank << " Time of Channel process: " << tChannel << endl;
