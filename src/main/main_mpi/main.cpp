...
int main(int argc, char *argv[]) {
	...
	if(rank == MASTER_RANK)
	{
		...
		if((size_t)nSlaves != groupSet.size())
		{
			...
			if((size_t)nSlaves != groupSet.size())
			{
			    ...
			    if((size_t)nSlaves != groupSet.size())
				{
			        cout << "The number of slave processes (" << nSlaves <<  ") is not consist with the group number(" << groupSet.size() << ").\n";
			        exit(-1);
				}
			}
		}
	...
	try
	{
		CalculateProcess(rank, numprocs, nSlaves, slaveComm, projectPath, modulePath, dbAddress.c_str(), dbPort, modelName.c_str(), nThread, layeringMethod);
	}
	catch(ModelException e)
	{   
	    cout << e.toString() << endl;
