...
rowComm()
{
    ...
    for (int i = 0; i < _pComVec->size(); i++)
    {
        ...
        if (_pMetadata->processor_number > 1)
        {
            ...
            int sendLowPoint =
                    (_pMetadata->_localworkBR.maxIRow() + 1 + _pNbrhood->minIRow()) * _pMetadata->_glbDims.nCols();
            ...
            else if (_pMetadata->myrank == (_pMetadata->processor_number - 1))
            {
                ...
                MPI_Recv(_matrix + recvUpPoint, upSize, datatype, _pMetadata->processor_number - 2, 1, MPI_COMM_WORLD,
                         &status);
            }
            else
            {
