...
def soil_parameters(dstdir, maskFile, sandList, clayList, orgList=None):
    ...
    for i in range(n):
        ...
        somFile = "%s/som%s.tif" % (dstdir, strLayer)
        fMask.write("%s\t%d\t%s\n" % (sandList[i], defaultSand, sandFile))
        fMask.write("%s\t%d\t%s\n" % (clayList[i], defaultClay, clayFile))    
        if orgList is not None:
            #fMask.write("%s\t%d\t%s\n" % (orgList[i], defaultSom, somFile))
            fMask.write("%s\t%f\t%s\n" % (orgList[i], defaultSom, somFile))   # modified by Zhu LJ,2015-04-01 
    fMask.close()

    s = "%s/mask_rasters/build/mask_raster %s" % (config.CPP_PROGRAM_DIR, configFile)
    os.system(s)
    ...
    for i in range(n):
        ...
        clayFile = "%s/clay%s.tif" % (dstdir, strLayer)
        somFile = "%s/som%s.tif" % (dstdir, strLayer)
        GenerateSoilAttributes(dstdir, i+1, sandFile, clayFile, somFile)
    SoilTexture(dstdir)
...
def landuse_parameters(dstdir, maskFile, inputLanduse, landuseFile, sqliteFile, defaultLanduse):
    ...
    s = "%s/reclassify/reclassify %s %s/reclassify/neigh.nbr" % (CPP_PROGRAM_DIR, reclassLuFile, CPP_PROGRAM_DIR)
    os.system(s)
...
#        fReclass.write("%s\t%d\n" % (landuseFile, 8))
#        fReclass.write("%s/reclassify/lookup\n" % (config.CPP_PROGRAM_DIR))
#        fReclass.write(dstdir + "\n")
#        attrList = config.CROP_ATTR_LIST
#        n = len(attrList)
#        fReclass.write("%d\n" % (n))
#        fReclass.write("\n".join(attrList))
#        fReclass.close()
#
#        s = "%s/reclassify/build/reclassify %s %s/reclassify/neigh.nbr" % (config.CPP_PROGRAM_DIR, reclassFile, config.CPP_PROGRAM_DIR)
#        os.system(s)
